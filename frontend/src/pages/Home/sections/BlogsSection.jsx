  ...
  return (
    <section style={{ padding: '8rem 0', background: 'var(--color-bg-alt)' }}>
      <div className="container">
        ...
        >
          {blogs.map((blog, i) => {
            const img = blog.coverImage || blog.cover_image || fallbackBlogs[i % fallbackBlogs.length].coverImage
            return (
              <motion.article 
              ...
              >
                ...
                <div style={{ padding: '2rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', fontSize: '0.875rem' }}>
                    ...
                    <span style={{ color: 'var(--color-text-muted)', fontSize: '0.75rem' }}>{blog.createdAt ? new Date(blog.createdAt).toLocaleDateString() : 'Guide'}</span>
                  </div>
                  ...
                  <Link to="/blogs" style={{ color: 'var(--color-primary)', fontWeight: '600', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none' }}>
