// EcoMargin Admin Panel — Factory & Plant Gallery CMS
...
export default function GalleryPage() {
  ...
  const fetchGallery = async () => {
    ...
    try {
      ...
      setError(err.message || err.data?.message || 'Failed to load gallery from database.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenEdit = (item) => {
    ...
    setFormData({
      title: item.title || '',
      category: item.category || 'Factory & Manufacturing',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.imageUrl.trim()) {
      setFormError('Image URL or uploaded image is required.');
      return;
    }
    ...
    try {
      if (editingItem) {
        await adminService.updateGallery(editingItem.id, formData);
        showToast('Gallery item updated successfully!');
      } else {
        await adminService.createGallery(formData);
      ...
      setFormError(err.message || err.data?.message || 'Failed to save gallery item');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>Factory & Plant Gallery CMS</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Manage Assembly Line Photos, SMT PCB Cleanrooms & Endurance Testing Labs</p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button onClick={fetchGallery} className="btn btn-outline" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div className="card" style={{ padding: '1rem', overflowX: 'auto' }}>
        {loading ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>Loading factory gallery from database...</div>
        ) : filteredGallery.length === 0 ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>No gallery items found in database.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-muted)' }}>
                <th style={{ padding: '0.75rem 1rem' }}>Preview</th>
                <th style={{ padding: '0.75rem 1rem' }}>Photo Caption / Title</th>
                ...
                return (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '1rem' }}>
                      {img ? (
                        <img src={img} alt={item.title} style={{ width: '50px', height: '50px', objectFit: 'cover', borderRadius: '4px' }} />
                      ) : (
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>No image</span>
                      )}
                    </td>
                    <td style={{ padding: '1rem', fontWeight: 600 }}>{item.title || 'Untitled Photo'}</td>
                    <td style={{ padding: '1rem' }}>
                    ...
                    <td style={{ padding: '1rem', textAlign: 'right' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                        <button onClick={() => handleOpenEdit(item)} className="btn btn-outline" style={{ padding: '0.4rem', color: 'var(--primary)' }} title="Edit">
                          <FiEdit2 />
                        </button>
          ...
          <div className="card" style={{ width: '100%', maxWidth: '550px', padding: '2rem', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                ...
                <input type="text" className="input" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} placeholder="e.g. Class 100,000 SMT Cleanroom Line" />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <select className="input" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}>
                    ...
                    <option value="EPC Installations">EPC Installations</option>
                  </select>
                </div>
                <div>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, marginBottom: '0.35rem' }}>Image File (Cloudinary Upload) *</label>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <input type="url" required className="input" value={formData.imageUrl} onChange={e => setFormData({ ...formData, imageUrl: e.target.value })} placeholder="https://res.cloudinary.com/..." />
                  <label htmlFor="gal-img-file" className="btn btn-outline" style={{ cursor: 'pointer', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                    <FiUploadCloud /> {uploading ? '...' : 'Upload'}
                  </label>
                  <input id="gal-img-file" type="file" accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
                ...
                <button type="submit" disabled={saving || uploading} className="btn btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  {saving ? 'Saving...' : editingItem ? <><FiSave /> Save Changes</> : <><FiPlus /> Add Photo</>}
                </button>
              </div>
            </form>
          </div>
